...
namespace pro079
{
	class ComandoManeador : IEventHandlerCallCommand
	{
		...
		private void MandaAyuda(Player player)
		{
			...
			"Para mtf, puedes poner la letra y el número de la unidad. Ejemplo: \".079 scp 173 mtf p 5\", si no lo haces, será aleatorio entre las unidades ya existentes\n" +
							...
							case "mtf":
								if (ev.Player.Scp079Data.AP >= 80)
								{
									if (ev.Player.Scp079Data.Level >= 1)
									{
										if (args.Length >= 5)
										{
											plugin.Info("MTF pillao como " + int.Parse(args[1]) + " " + int.Parse(args[2]) + " " + args[3][0]);
											/*
											...
											else */PluginManager.Manager.Server.Map.AnnounceNtfEntrance(int.Parse(args[1]), int.Parse(args[2]), args[3][0]);
										}
										else
										{
											ev.Player.SendConsoleMessage("Uso: .079 mtf (p) (5) (4), dirá que Papa-5 viene y quedan 4 SCP");
