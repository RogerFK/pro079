						...
						switch (args[0])
						{
							...
							case "info":
								...
								if (ev.Player.Scp079Data.Level > 0 || ev.Player.GetBypassMode())
								{
									...
									if (DeconBool == true) tiempoDecont = "La descontaminación está desactivada";
									else if (PluginManager.Manager.Server.Map.LCZDecontaminated == true) tiempoDecont = "Ya ha ocurrido descontaminación";
									else tiempoDecont = (DeconTime - float.Parse(PluginManager.Manager.Server.Round.Duration.ToString()) / 60.0f).ToString("0.00");
								}
								...
								{
									if (LastMtfSpawn - PluginManager.Manager.Server.Round.Duration < MinMTF) MTFtiempo = "entre " + (MinMTF - PluginManager.Manager.Server.Round.Duration + LastMtfSpawn).ToString("0") + " y " + (MaxMTF - PluginManager.Manager.Server.Round.Duration + LastMtfSpawn).ToString("0");
									else MTFtiempo = "menos de " + (MaxMTF - PluginManager.Manager.Server.Round.Duration + LastMtfSpawn).ToString("0"); 
								}
								...
								foreach (Generator generator in PluginManager.Manager.Server.Map.GetGenerators())
								{
									...
									if (generator.Engaged) ev.ReturnMessage = ev.ReturnMessage + " está activado.\n";
									else ev.ReturnMessage = ev.ReturnMessage + (generator.HasTablet ? " tiene una tablet y le quedan " : " no tiene tablet y le quedan " ) + generator.TimeLeft.ToString("0") + " segundos.\n";
								}
